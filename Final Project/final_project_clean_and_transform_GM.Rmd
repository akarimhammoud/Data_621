...
Subset `data` by `Type` to focus on dogs *woof*.
```{r}
...
Below we create a dataframe using the list of [hypoallergenic](https://www.akc.org/dog-breeds/hypoallergenic-dogs/) dogs from the American Kennel Club. Once created, we `left_join()` it to our `dog_data` dataframe to add the new feature for analysis.
...
breed <- c("Affenpinscher", "Afgan Hound", "American Hairless Terrier", 
                    ...
                    "Lagotto Romangnolo", "Lowchen", "Maltese","Miniature Shnauzer",
                    "Peruvian Inca Orchid", "Poodle", "Standard Poodle", "Schnauzer",
                    "Portuguese Water Dog", "Russian Tsvetnaya Bolonka", "Wheaten Terrier",
...
```{r}
dog_data_merge <- left_join(dog_data, hypoallergenic, by=c("breedname" = "breed"))

dog_data_merge$hypoallergenic[is.na(dog_data_merge$hypoallergenic)] <- "No"

#need to add NA back for Mixed Breed as some mixed breeds can be hypoallergenic 

dog_data_merge$hypoallergenic <- as.factor(dog_data_merge$hypoallergenic)
```
