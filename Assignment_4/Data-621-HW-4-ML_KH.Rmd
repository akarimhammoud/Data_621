...
model1 <- glm(TARGET_AMT ~ 1, data = df_ins, family = "quasipoisson")
summary(model1)
```

#### Quasipoisson FULL Model

```{r, warning=FALSE}

model2 <- glm(TARGET_AMT ~ ., data = df_ins, family = "quasipoisson")
...
model3 <- glm(TARGET_AMT ~ ., data = df_ins)
...
```{r, warning=FALSE}
stepWise_b <- stepAIC(model3, direction = "backward")
```
