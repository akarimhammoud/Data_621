...
df_ins_mrm<-na.exclude(df_ins_mrm)
  
```

### Baseline Models

First, we will begin by creating our base models by modelling the response variable `TARGET_AMT_log` to all the features and null model.

```{r}
#null model
null.model <- lm(TARGET_AMT_log ~ NULL, data = df_ins_mrm)
 
summary(null.model)
```

```{r message=FALSE, warning=FALSE}
#fit the lm model to all variables with no adjustments
full.model <- lm(TARGET_AMT_log ~., data = df_ins_mrm)
...
Using the `full.model`, we create `lm.model` using only the significant variables previously identified.
```{r}
set.seed(123)
lm.model <- lm(TARGET_AMT_log ~  KIDSDRIV + PARENT1 + MSTATUS + CAR_USE +
                CAR_TYPE + REVOKED + URBANICITY +BLUEBOOK_cbrt +
                OLDCLAIM_cbrt +HOME_VAL_sqrt, 
               data = df_ins_mrm)
...
```{r}
RMSE <- sqrt(mean(lm.model$residuals^2))
RMSE
```
...
```{r}
ols_plot_resid_hist(lm.model)
```

### Stepwise Regression Model
```{r}
set.seed(456)
step.model <- stepAIC(full.model, trace = F, direction = 'backward')
summary(step.model)
```
...
```{r}
#root mean sq err
RMSE <- sqrt(mean(step.model$residuals^2))
RMSE
```
...
```{r}
ols_plot_resid_hist(step.model)
```

### Weighted Least Squares Stepwise Regression Model

```{r}
set.seed(789)
#get the residuals from the full model
res_sq <- full.model$residuals^2

#specify the weights argument
wls.model <- lm(TARGET_AMT_log ~ ., data = df_ins_mrm, weights = 1/res_sq)
wls.step.model <- stepAIC(wls.model, trace = F, direction = 'backward')
summary(wls.step.model)
```

```{r}
#root mean sq err
RMSE <- sqrt(mean(wls.step.model$residuals^2))
RMSE
...
```{r}
ols_plot_resid_hist(wls.step.model)
```
...
Using the RMSE as our selection metric, of the three models produced above we find that the best of the three is the Stepwise Regression Model based on the RMSE.
...
```{r}
#predictions with the transformed variables
predictions <- predict(step.model, df_eval_mrm, type = "response")
summary(predictions)
```

Histogram of Predicted `TARGET_AMT`
```{r}
#this depends on the predictions variable
predicion_df <- as.data.frame(predictions) #create df for ggplot

predicion_df$values <- round((exp(predicion_df$predictions)),4)

head(predicion_df$values)

ggplot(predicion_df, aes(x=values)) + 
 geom_histogram(aes(y=..density..), colour="black", fill="white")+
 geom_density(alpha=.2, fill="steelblue")+
labs(title="Predicted Value",x="TARGET_AMT", y = "Density")
```

Histogram of Actual `TARGET_AMT`

```{r}
ggplot(df_ins, aes(x=TARGET_AMT)) + 
 geom_histogram(aes(y=..density..), colour="black", fill="white")+
 geom_density(alpha=.2, fill="steelblue")+
labs(title="Actual Value",x="TARGET_AMT", y = "Density")
```
