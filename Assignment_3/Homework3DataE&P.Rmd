...
Lastly, we proceed to check if there are any missing data point in the `crime_train`
...
#convert chas and target to target factors
...
```{r}
fit <- glm(target~., family = "binomial", data = train)
...
```{r}
fit_null <- glm(target~1,data=train, family="binomial")
summary(fit_null)
```


To determine if the full model has significant improvement in fit over the null model, we make use of the `anova()` function.
```{r}
